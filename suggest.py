...
def getExerSuggestion(equipments, filePath=FILENAME):
    ...
    for equipment in equipments:
        ...
        allEquipExer[equipment] = {
            "topExer": [topExer, topDes, topOutput],
...
def getExer(
    exerList, exerDesc, exerDiffList, exerCount, exerTime, n=3, condition=False
):
    topExer = []
    hardExer = []
    mediumExer = []
...
def suggestExerCount(exers, properties, minute=EASY):
    ...
    for i in range(exerCount):
        ...
        setStr = (
            f"{int(sets)}" if sets <= floor(sets) else f"{floor(sets)} - {ceil(sets)}"
        )
    ...
    for equipment in allEquipExer:
        ...
        if len(equipExer["easyExer"][0]):
            ...
            for i in range(len(equipExer["easyExer"][0])):
                easy.append(
                    f'{equipExer["easyExer"][0][i]} {equipExer["easyExer"][2][i]}\n'
                )
        ...
        if len(equipExer["mediumExer"][0]):
            ...
            for i in range(len(equipExer["mediumExer"][0])):
                medium.append(
                    f'{equipExer["mediumExer"][0][i]} {equipExer["mediumExer"][2][i]}\n'
                )
        ...
        if len(equipExer["hardExer"][0]):
            ...
            for i in range(len(equipExer["hardExer"][0])):
                hard.append(
                    f'{equipExer["hardExer"][0][i]} {equipExer["hardExer"][2][i]}\n'
                )
